...
womp::WompRenderer::WompRenderer(Window& windowRef): m_framesInFlight(Swapchain::MAX_FRAMES_IN_FLIGHT) {
    ...
    m_testImage = std::make_unique<Image>(
        deviceRef,
        "resources/kobeSilly.png",
        VK_FORMAT_R8G8B8A8_SRGB,
        VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
        VMA_MEMORY_USAGE_GPU_ONLY, VK_FILTER_LINEAR
    );
}
...
void womp::WompRenderer::drawTexture(const TextureHandle image, float x, float y, float width, float height, const glm::vec4& color) {
    m_pendingDrawCommands.push_back(DrawCommand{
        .texture = image,
        .dstRect = glm::vec4(x, y, width, height),
        .color = color
    });
}

void womp::WompRenderer::drawTexture(TextureHandle image, glm::vec2 position, glm::vec2 size, float rotation, const glm::vec4& color) {
    m_pendingDrawCommands.push_back(DrawCommand{
        .texture = image,
        .dstRect = glm::vec4(position.x, position.y, size.x, size.y),
        .rotation = rotation,
        .color = color
    });
}
...
void womp::WompRenderer::render() {
    if (const VkCommandBuffer commandBuffer = m_renderer->BeginFrame()) {
        ...
        for (const auto& cmd: m_pendingDrawCommands) {
            ...
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 0, 1, &tex.descriptorSet, 0, nullptr);
...
womp::TextureHandle womp::WompRenderer::createTexture(const std::string& filepath) {
    ...
    m_descriptorPool->allocateDescriptor(m_descriptorSetLayout->getDescriptorSetLayout(), set);
